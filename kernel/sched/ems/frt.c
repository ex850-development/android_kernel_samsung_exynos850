...
/* HACK: it must be removed, just temporarily */
void frt_limit_fast_cpu(int enable)
{
	struct frt_dom *dom;

	list_for_each_entry(dom, &frt_list, list) {
		if (cpumask_test_cpu(4, &dom->cpus))
			dom->active_ratio = enable ? 100 : 3;

		if (cpumask_test_cpu(6, &dom->cpus))
			dom->active_ratio = enable ? 100 : 5;
	}
}
